...
| Закладка          | Функционал                                                                       |
|-------------------|----------------------------------------------------------------------------------|
| **В работе**      | Текущие задачи в работе                                                          |
| **Запланировано** | Запланированные, но ещё не начатые задачи                                        |
| **База знаний**   | Материалы для изучения, по которым ещё пока не сформирован детализированный план |
...
```text
┌──────────────────────────────────────────────-------------------------────────┐
|               🏷️ Индивидуальный план развития                                 |
├──────────────────────────────────────────────-------------------------────────┤
| [В работе]  [Запланировано]  [База знаний]  [Источник]                        |
├────────────------------------------------------------──┬──────────────────────┤
|                                                        | 📊 Статистика        |
|                                                        | 🕰️ История           |
|                                                        | 📂 Примеры планов    |
|  Содержание выбранной вкладки                          |                      |
|                                                        |                      |
|                                                        |                      |
└───────────------------------------------------------───┴──────────────-───────┘
```
...
Текущие задачи, над которыми ведётся работа.
...
graph LR
    ...
    Карточка --> Раздел
    Карточка --> Порядок
    Карточка --> Дата_начала
    Карточка --> Планируемая_дата_выполнения
    Карточка --> Прогресс[🟩🟩🟩⬜⬜ 60%]
    Карточка --> Срочность[⚠️ Если просрочено]
```

**Функционал:**

- Сортировка по порядку, который задается в карточках.
- Клик → открытие связанного `.md`-файла.
- Прогресс-бар под каждой карточкой.
- **Предупреждение "Слишком много задач в работе, верните часть из них в Очередь"**, если задач > 5 (настраивается).
- **Подсветка красным "Просрочено"**, если задача просрочена.
...
| Название              | Раздел           | Порядок |
|-----------------------|------------------|---------|
| "Martin - Clean Code" | Программирование | 1       |
| ...                   | ...              | ...     |
...
```mermaid
flowchart LR
    A[📚 Добавлено в Базу знаний] --> B[📝 Запланировано]
    B --> C[🛠️ Взято в работу]
    C --> D[✅ Выполнено и архивировано в Истории]
```
...
Выберите удобный язык работы:

- English (по умолчанию)
- Русский

_Изменения применяются после перезагрузки плагина_

### Ограничения задач

Настройте комфортное количество задач в работе:

- **Максимум активных задач**: 5
- **Предупреждение**: появляется при 5+ задачах
...
**Начало отсчета**: Укажите дату, с которой собирать данные. По умолчанию — дата установки плагина.
...
1. Нажмите "+ Новый тип"
2. Укажите название (например, "Вебинар")

### Порядок отображения

Перетащите типы в нужном порядке — так они будут показываться в интерфейсе.

### Возможность отключения

У каждого типа есть возможность отключить его отображение в плане.

### Дополнительные пункты в плане выполнения задачи

Для каждого типа есть возможность настроить пункты, которые будут попадать в детализированный план при создании карточки заданного типа.

## Разделы и категории

- Добавление произвольного раздела
- Сортировка разделов
...
| Период        | Включить | Примеры задач                                               |
|---------------|----------|-------------------------------------------------------------|
| Ежедневно     | ✓        | Утренний обзор почты; Прочитать главу из книги              |
| Еженедельно   | ✓        | Решить задачу на Project Euler; Прослушать подкаст "Culips" |
| Ежемесячно    | ✓        | Посмотреть видео с доклада ScalaConf                        |
| Ежеквартально | ✓        | Добавить в Базу знаний книги, рекомендованные на ScalaConf  |
| Ежегодно      | ✓        | Закончить курс "Продвинутая Scala"                          |
