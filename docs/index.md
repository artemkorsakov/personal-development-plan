...
**Personal Development Plan** — плагин для Obsidian, 
предназначенный для структурированного планирования личного и профессионального развития. 
Помогает организовать задачи по обучению, отслеживать прогресс и управлять ресурсами.

---

## Обзор плагина

![Основные вкладки](images/main_tabs.png)

### Основные вкладки

- 🛠️ **В работе** — текущие активные задачи
- ⏳ **Запланировано** — задачи, запланированные к выполнению
- 📚 **База знаний** — материалы для изучения без детального плана
...
Отображает текущие активные задачи.

![Интерфейс вкладки](images/inProgress.png)

#### Основные функции

1. **Сортировка задач**
	- Автоматическая сортировка по полю `order`
...
2. **Навигация к задаче**
	...
	- Путь к папке задач задаётся в [настройках](settings.md)

3. **Визуализация прогресса**
	- Индикатор выполнения:
		- Графический прогресс-бар
		- Точное процентное значение (например "45%")
	- Автоматическое обновление при изменении файла

4. **Система предупреждений**
	- Предупреждение при превышении лимита задач:
	  ```
	  ⚠️ Слишком много задач в работе, верните часть из них в Очередь (5 > 3)
	  ```
	- Исчезает при приведении количества задач в норму

5. **Контроль сроков**
	- Просроченные задачи помечаются:
		- Красным цветом даты
		- Текстом "Просрочено"
		- Восклицательным знаком в иконке
...
4. **Метка порядка**
	...
	- Соответствует полю `order` в файле задачи

#### Рекомендации

1. Для изменения порядка редактируйте поле `order`
2. Для срочных задач устанавливайте высокий приоритет (меньший порядковый номер)

---

### Вкладка "Запланировано"

Хранит задачи, запланированные к выполнению.

![Интерфейс вкладки](images/planned.png)

#### Основные функции

1. **Вкладки типов**
	- Группировка по типам материалов (книги, курсы и др.)
	- Цифры в скобках — количество задач каждого типа

2. **Сортировка задач**
	- По полю `order` (настраивается для каждой задачи)

3. **Навигация**
	- Клик по карточке открывает полное описание задачи

#### Рекомендации

- Используйте поле `order` для расстановки приоритетов
- Для скрытия типов задач отключите их в настройках
- Добавляйте теги для удобства поиска

---

### Вкладка "База знаний"

Содержит материалы для изучения без детального плана.
...
| Название              | Тип    | Раздел           |
|-----------------------|--------|------------------|
| "Martin - Clean Code" | Книга  | Программирование |
| ...                   | ...    | ...              |

#### Функционал

- Сортировка задач по названию
- Клик по названию открывает `.md`-файл
- Кнопка **"Экспорт в JSON"** для обмена базы знаний с сообществом
