...
2. **Навигация к задаче**
    - Клик по любой области карточки открывает связанный Markdown-файл задачи (путь к папке задач задается в [настройках](settings.md))
...
**Здесь хранятся все задачи, которые вы планируете выполнить.**  
Отсюда их можно быстро перевести в работу или отредактировать.

#### 🔍 Как это выглядит

![Интерфейс закладки "Запланировано"](images/planned.png)

1. **Вкладки** сверху – типы материалов (книги, курсы и др.)
2. **Цифры в скобках** – количество задач каждого типа
3. **Карточки задач** – кликабельные блоки с краткой информацией

✅ **Сортировка задач**

- По порядку (изменяется в настройках каждой задачи)

✅ **Быстрый доступ**

- Клик на карточку → открытие полного описания задачи

#### 💡 Полезные советы

• Используйте поле **order**, чтобы расставить приоритеты  
• Чтобы скрыть тип задач (например, подкасты), отключите его в **настройках плагина**  
• Для быстрого поиска добавляйте **теги** в описание задач
